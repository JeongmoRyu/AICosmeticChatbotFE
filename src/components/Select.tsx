  ...
  return (
    <div ref={selectboxRef} className={`selectbox ${selectboxClass}`}>
      <select {...props.register} id={props.id} name={props.name} defaultValue={selectedValue}>
        ...
        {props.typeList && props.typeList.map((type) => {
          return <option 
            key={`select_${type.value}`} 
            value={type.value} 
            disabled={type.disabled} 
            className={`${type.disabled && '!bg-secondary-gray'}`}>{type.label}</option>
        })}
      </select>
      <button 
        ref={selectedBtnRef} 
        ...
        onClick={handleSelectList}>{selectedLabel}</button>
      <div className={`select-items bg-white ${isSelectHide}`}>
        {props.placeholder && <button type="button" data-value="" data-label={props.placeholder} onClick={handleOnChangeSelect}>{props.placeholder}</button>}
        {props.typeList && props.typeList.map((item) => {
          return <button 
            ...
            className={`${item.disabled && '!bg-secondary-gray !text-[#949CA5]'}`}
