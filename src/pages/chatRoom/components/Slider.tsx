  ...
  return (
    <div className='flex items-center w-full justify-between min-w-0'>
      <button className='flex items-center justify-center w-5 h-5' onClick={handleLeftClick}>
        <img src={isAtStart ? left_arrow_inactive : left_arrow} alt='left' />
      </button>
      <div
        className={`slider-container flex items-center overflow-x-auto scrollbar-hide relative m-5 ${width}`}
        ref={sliderRef}
      >
      ...
      <button className='flex items-center justify-center w-5 h-5' onClick={handleRightClick}>
        <img src={isAtEnd ? right_arrow_inactive : right_arrow} alt='right' />
      </button>
    </div>
  );
}
